...
export default function SubCategoryPage(props: { params: Promise<{ categories: string, subcategories: string }> }) {
    const { categories, subcategories } = use(props.params)
    const router = useRouter()
    const [selectedImage, setSelectedImage] = useState<string | null>(null);

    const mainCategory = categoriesJson.categories.find((cat) => cat.category === categories);
    const subCategory = mainCategory?.subCategories?.find((cat) => cat.category === subcategories);
                ...
                <div className="text-center max-w-md mx-auto">
                    <div className="relative mb-8">
                        <div className="w-24 h-24 mx-auto bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center mb-6 animate-pulse">
                            <Sparkles className="w-12 h-12 text-white" onClick={() => router.push("/categories")} />
                        </div>
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 auto-rows-[200px] md:auto-rows-[250px]">
                    {subCategory.images.map((image, index) => {
                        const layoutClass = clothLayouts[Math.min(subCategory.images!.length, 6) as keyof typeof clothLayouts]?.[index % clothLayouts[Math.min(subCategory.images!.length, 6) as keyof typeof clothLayouts].length] || "col-span-1 row-span-1";
